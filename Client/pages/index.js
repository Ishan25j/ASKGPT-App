...
const Home = ({ getChats, reqErr }) => {
  ...
  const eventCard = chats.map(chat => {
    var dateObtained = chat.date;
    if ( chat.date[chat.date.length - 1] === 'Z') {
      dateObtained = chat.date.slice(0, -1);
    }
    if (chat.ticketsLeft > 0 && (new Date(dateObtained) - new Date()) > 0) {
      return (
        <div className="col-sm events" key={chat.id}>
